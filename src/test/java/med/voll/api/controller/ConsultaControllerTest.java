...
class ConsultaControllerTest {
    ...
    @Autowired
    private JacksonTester<DatosAgendarConsulta> datosAgendarConsultaJson;
    @Autowired
    private JacksonTester<DatosDetalleConsulta> datosDetalleConsultaJson;
    @MockitoBean
    private AgendamentoDeConsultas agendaDeConsultas;

    @Test
    @DisplayName("Deberia devolver http 400 cuando la request no tenga datos")
    @WithMockUser
    void agendar_escenario1() throws Exception {
    ...
    @WithMockUser
    void agendar_escenario2() throws Exception {
        var fecha = LocalDateTime.now().plusHours(1);
        var especialidad = Especialidad.CARDIOLOGIA;
        var datosDetalle = new DatosDetalleConsulta(null, 2l, 5l, fecha);
        when(agendaDeConsultas.agendarConsulta(any())).thenReturn(datosDetalle);

        var response = mockMvc.perform(post("/consultas")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(datosAgendarConsultaJson.write(
                                new DatosAgendarConsulta(2l,5l, fecha,especialidad)).getJson()
                        )
                )
                .andReturn().getResponse();
